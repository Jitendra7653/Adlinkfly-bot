...
# Optional variables
LOG_CHANNEL = int(os.environ.get("LOG_CHANNEL", "-1002126197074"))  # log channel for information about users
UPDATE_CHANNEL = int(os.environ.get("UPDATE_CHANNEL", "-1001976505683"))  # For Force Subscription
BROADCAST_AS_COPY = is_enabled(os.environ.get("BROADCAST_AS_COPY", "False"), False)  # true if forward should be avoided
IS_PRIVATE = is_enabled(os.environ.get("IS_PRIVATE", "true"), True)  # true for private use and restricting users
SOURCE_CODE = os.environ.get("SOURCE_CODE", "https://allin1tricks.online")  # for upstream repo
...
FORWARD_MESSAGE = is_enabled(os.environ.get("FORWARD_MESSAGE", "False"), False)  # true if forward message to converted by reposting the post
